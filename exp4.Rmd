---
title: "Hidden vs Visible Resources in the Survival Game (Study 4)"
...
Now, read in the data from Study 4.

```{r}
(
  d <- 
    read.csv('data/exp4data.csv', header = TRUE) %>%
    as_tibble() 
)
```
...
save(b4.1, file = "models/exp4_brmsfit1.rda")
```

```{r echo=F}
load("models/exp4_brmsfit1.rda")
```
...
save(b4.2, file = "models/exp4_brmsfit2.rda")
```

```{r echo=F}
load("models/exp4_brmsfit2.rda")
```
...
save(b4.3, file = "models/exp4_brmsfit3.rda")
```

```{r echo=F}
load("models/exp4_brmsfit3.rda")
```
...
Bayes factor for the difference between probabilities being equal to zero.

```{r}
(1 / hypothesis(b4.3, "inv_logit_scaled(intercept + Condition) - inv_logit_scaled(intercept) = 0", seed = 2113)$hypothesis$Evid.Ratio) %>% round(2)
```
...
```{r}
bayes_R2(b4.3) %>% round(2)
```

This model explains around 33% of the variance in requesting behaviour.

```{r echo=F}
# cleanup
rm(d.trim, post, difference, hidden_prob, visible_prob)
```
...
save(b4.4, file = "models/exp4_brmsfit4.rda")
```

```{r echo=F}
load("models/exp4_brmsfit4.rda")
```
...
```{r}
(1 / hypothesis(b4.4, "inv_logit_scaled(intercept + Condition) - inv_logit_scaled(intercept) = 0", seed = 2113)$hypothesis$Evid.Ratio) %>% round(2)
```
...
```{r}
bayes_R2(b4.4) %>% round(2)
```

This model explains around 31% of the variance in the outcome.
...
save(b4.5, file = "models/exp4_brmsfit5.rda")
```

```{r echo=F}
load("models/exp4_brmsfit5.rda")
```
...
```{r}
(1 / hypothesis(b4.5, "inv_logit_scaled(intercept + Condition) - inv_logit_scaled(intercept) = 0", seed = 2113)$hypothesis$Evid.Ratio) %>% round(2)
```

This Bayes Factor provides weak support for the hypothesis that the probabilities differ.

```{r}
bayes_R2(b4.5) %>% round(2)
```

Finally, `bayes_R2()` tells us that this model explains around 32% of the variance in the outcome.
...
save(b4.6, file = "models/exp4_brmsfit6.rda")
```

```{r echo=F}
load("models/exp4_brmsfit6.rda")
```

Here are the priors we used for model `b4.6`.
...
```{r}
(1 / hypothesis(b4.6, "inv_logit_scaled(intercept + Condition) - inv_logit_scaled(intercept) = 0", seed = 2113)$hypothesis$Evid.Ratio) %>% round(2)
```

This Bayes Factor provides anecdotal support for the alternative (difference) over the null (no difference) hypothesis.

```{r}
bayes_R2(b4.6) %>% round(2)
```

34% of the variance is explained by this model.
...
```{r echo=F, fig.height = 3, fig.width = 9.5}
# prepare for gA
post <- posterior_samples(b4.6)
visible_prob <- inv_logit_scaled(post$b_intercept)
hidden_prob  <- inv_logit_scaled(post$b_intercept + post$b_Condition)
...
gA <-
  ...
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 2.06"), size = 3.5) +
  ylim(0,1) + 
...
d.plot <-
  ...
  summarise(alive          = sum(!is.na(herd_size_after_shock)) / n(),
            requested      = sum(requested, na.rm = T) / n(),
            ableToFulfill  = sum(ableToFulfill, na.rm = T) / n(),
            noFulfillWhenAble = sum(noFulfillWhenAble, na.rm = T) / n())
...
gC <-
  ...
  scale_x_continuous('Round number', c(5, 10, 15, 20, 25)) +
  scale_y_continuous(breaks = c(seq(0, 1, 0.25)), limits = c(0, 1), position = "right") +
  ylab('Proportion of Player 2s alive') +
  theme_classic() +
  theme(panel.grid.major.x = element_blank(),
        axis.title = element_text(size = 10),
        axis.text = element_text(size = 8),
...
g <- plot_grid(gA, NULL, gB, gC, legend, nrow = 1, 
                 labels = c("a","","b","",""), 
                 rel_widths = c(0.7, 0.1, 1, 1, 0.25))
g

# save to working directory
ggsave('figures/fig4.pdf', width = 9.5, height = 3)
