...
gA <-
  ...
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 11.86"), size = 3.5) +
  ylim(0,1) + 
...
d.plot <-
  ...
  summarise(alive        = sum(!is.na(herd_size_after_shock)) / n(),
            above        = sum(above, na.rm = T) / n(),
            request      = sum(request, na.rm = T) / n(),
            requestAbove = sum(requestAbove, na.rm = T) / n())
...
gC <-
  ...
  ylab('Proportion of players alive') +
  theme_classic() +
  theme(panel.grid.major.x = element_blank(),
        axis.title = element_text(size = 10),
        axis.text = element_text(size = 8),
...
gD <-
  ...
  geom_ribbon(aes(ymin = pi.025, ymax = pi.975), alpha = 0.2) +
  geom_ribbon(aes(ymin = pi.250, ymax = pi.750), alpha = 0.2) +
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 2.79"), size = 3.5) +
  ylim(0,1) + 
