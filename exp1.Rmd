...
Bayes Factor for difference between conditions.
...
There's only anecdotal evidence that it does.
...
```{r echo=F}
quantile(post$b_Intercept + post$b_request, c(.025, .5, .975)) %>% round(2)
```
...
```{r echo=F}
quantile(post$b_Intercept + post$b_Condition + post$b_request + post$`b_Condition:request`, 
         c(.025, .5, .975)) %>% round(2)
```

Does the herd size when requesting differ across conditions?

```{r echo=F}
sum(post$b_request + post$`b_Condition:request` < 0) / 8000
```

Yes. 100% of the posterior difference is above zero, indicating that participants in the Hidden condition have more resources when they request from their partner than participants in the Visible condition.
...
```{r echo=F}
post <- readd(post1.11)
quantile(exp(post$b_Intercept), c(.025, .5, .975)) %>% round(2)
```

And in the Hidden condition?

```{r echo=F}
quantile(exp(post$b_Intercept + post$b_Condition), c(.025, .5, .975)) %>% round(2)
```
...
Yes. Participants in the Hidden condition ask for more cattle when they request.
...
```{r echo=F}
sum(post$b_Condition < 0) / 8000
```

These amounts differ across conditions.
