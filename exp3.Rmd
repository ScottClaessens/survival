...
gA <-
  ...
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 3.94"), size = 3.5) +
  ylim(0,1) + 
...
gB <-
  ...
  geom_ribbon(aes(ymin = pi.025, ymax = pi.975), alpha = 0.2) +
  geom_ribbon(aes(ymin = pi.250, ymax = pi.750), alpha = 0.2) +
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 14.40"), size = 3.5) +
  ylim(0,1) + 
  labs(y = 'Median predicted probability of\nnot fulfilling request when able',
       x = 'Condition',
       title = ' ') +
  theme_classic() +
  theme(panel.grid.major.x = element_blank(),
...
gC <-
  ...
  geom_ribbon(aes(ymin = pi.025, ymax = pi.975), alpha = 0.2) +
  geom_ribbon(aes(ymin = pi.250, ymax = pi.750), alpha = 0.2) +
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = "BF = 0.96"), size = 3.5) +
  ylim(0,1) + 
  labs(y = 'Median predicted probability of\nnot fulfilling request when able',
       x = 'Condition',
...
# finished plot!
g <- plot_grid(gA, gB, gC, nrow = 1, labels = letters[1:3])
g

# save to working directory
ggsave('figures/fig3.pdf', width = 9, height = 4)

# cleanup
rm(gA, gB, gC, post, hidden_prob, visibleAsk_prob,
