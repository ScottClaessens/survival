...
```{r echo=F}
load("models/exp2_brmsfit4b.rda")
```
...
```{r echo=F}
# cleanup
rm(d.trim, post, difference_VisibleFirst, difference_HiddenFirst,
   hidden_VisibleFirst_prob, hidden_HiddenFirst_prob,
   visible_VisibleFirst_prob, visible_HiddenFirst_prob)
```
...
```{r echo=F}
# cleanup
rm(post, difference, visible_prob, hidden_prob)
```
...
```{r echo=F}
load("models/exp2_brmsfit6b.rda")
```
...
```{r echo=F}
# cleanup
rm(d.trim, post, difference_HiddenFirst, difference_VisibleFirst,
   hidden_HiddenFirst_prob, hidden_VisibleFirst_prob,
   visible_HiddenFirst_prob, visible_VisibleFirst_prob)
```
...
gA <-
  ...
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = " Hidden first: BF = 3.05"), 
...
gD <-
  ...
  geom_point(aes(colour = order)) + 
  geom_ribbon(aes(ymin = pi.025, ymax = pi.975, fill = order), alpha = 0.075) +
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 1.5, label = " Hidden first: BF = 7.07"), 
            colour = "#00BFC4", size = 3) +
  geom_text(aes(x = Inf, y = Inf, hjust = 1, vjust = 3.5, label = "Visible first: BF = 1.43"), 
            colour = "#F8766D", size = 3) +
  ylim(0,1) + 
  labs(y = 'Median predicted probability of\not fulfilling requests when able',
       x = 'Condition') +
  theme_classic() +
  theme(panel.grid.major.x = element_blank(),
        axis.title = element_text(size = 10),
...
# cleanup
rm(gA, gB, gC, gD, gE, gF, d.plot, top, bottom, legend, post,
   visible_HiddenFirst_prob, visible_VisibleFirst_prob,
   hidden_HiddenFirst_prob, hidden_VisibleFirst_prob)
```
